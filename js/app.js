...
function updateClock() {
  const now = new Date();
  document.getElementById('headerDate').textContent = now.toLocaleDateString('de-DE', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }) + ' · ' + now.toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit' }) + ' Uhr';
}
updateClock();
...
const sunIconSvg = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="12" cy="12" r="4.2"/><path d="M12 3v2.2M12 18.8V21M4.2 4.2l1.5 1.5M18.3 18.3l1.5 1.5M3 12h2.2M18.8 12H21M4.2 19.8l1.5-1.5M18.3 5.7l1.5-1.5" stroke-linecap="round"/></svg>';
const moonIconSvg = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M20.5 14.8A8.5 8.5 0 1 1 9.2 3.5a6.8 6.8 0 0 0 11.3 11.3z"/></svg>';

function getStoredTheme() {
  try { return localStorage.getItem('dashboard-theme'); } catch (err) { return null; }
}
function setStoredTheme(v) {
  try { localStorage.setItem('dashboard-theme', v); } catch (err) {}
}
...
function setStoredAutoRefresh(v) {
  try { localStorage.setItem('dashboard-auto-refresh', String(v)); } catch (err) {}
}
...
pickerOverlay.addEventListener('click', e => { if (e.target === pickerOverlay) closeWidgetPicker(); });
...
function removeWidget() {
  try { localStorage.removeItem('dashboard-widget-slot'); } catch (err) {}
...
function mountWidget(type, innerHtml) {
  try { localStorage.setItem('dashboard-widget-slot', type); } catch (err) {}
  const panel = document.createElement('section');
  panel.className = 'panel widget-panel';
  panel.id = 'widgetSlot';
  panel.innerHTML = `<button type="button" class="widget-remove" aria-label="Widget entfernen">×</button>` + innerHtml;
  ...
  if (header) {
    ...
    toggleBtn.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 9l6 6 6-6"/></svg>';
...
function mountTodoWidget() {
  ...
  function loadTodos() {
    try { return JSON.parse(localStorage.getItem('dashboard-widget-todo') || '[]'); }
    catch (err) { return []; }
  }
  function saveTodos(items) {
    try { localStorage.setItem('dashboard-widget-todo', JSON.stringify(items)); } catch (err) {}
  }
  let todos = loadTodos();
...
function mountNotesWidget() {
  ...
  try { area.value = localStorage.getItem('dashboard-widget-notes') || ''; } catch (err) {}
...
function mountClockWidget() {
  ...
  function tickClocks() {
    ...
    list.querySelectorAll('.clock-row').forEach(row => {
      ...
      const localParts = new Intl.DateTimeFormat('en-US', { hourCycle: 'h23', hour: '2-digit', minute: '2-digit' }).formatToParts(now);
      ...
      row.querySelector('[data-diff]').textContent = diffMin === 0 ? 'Ortszeit' : (diffH > 0 ? `+${diffH}h` : `${diffH}h`);
    });
  }
  tickClocks();
...
function mountCountdownWidget() {
  const panel = mountWidget('countdown', `
    <div class="widget-panel-header">
  ...
  function loadCountdown() {
    try { return JSON.parse(localStorage.getItem('dashboard-widget-countdown') || 'null'); }
    catch (err) { return null; }
  }
  function saveCountdown(data) {
    try { localStorage.setItem('dashboard-widget-countdown', JSON.stringify(data)); } catch (err) {}
  }
  ...
  function renderCountdown() {
    ...
    body.querySelector('#cdEdit').addEventListener('click', () => {
      try { localStorage.removeItem('dashboard-widget-countdown'); } catch (err) {}
      renderSetup();
    });
    ...
    function tick() {
      ...
      if (!numberEl) { clearInterval(countdownTimer); return; }
...
function mountWarningsWidget() {
  const panel = mountWidget('warnings', `
    <div class="widget-panel-header">
  ...
  async function loadWarnings(filterText) {
    ...
    try {
      ...
      all.sort((a, b) => (severityRank[b.severity] || 0) - (severityRank[a.severity] || 0) || new Date(b.startDate) - new Date(a.startDate));
      ...
      all.forEach(w => {
        ...
        const date = w.startDate ? new Date(w.startDate).toLocaleString('de-DE', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' }) : '';
        item.innerHTML = `
...
function mountChecklistWidget() {
  ...
  const panel = mountWidget('checklist', `
    <div class="widget-panel-header">
  ...
  function loadChecked() {
    try { return JSON.parse(localStorage.getItem('dashboard-widget-checklist') || '[]'); }
    catch (err) { return []; }
  }
  function saveChecked(arr) {
    try { localStorage.setItem('dashboard-widget-checklist', JSON.stringify(arr)); } catch (err) {}
  }
...
(function restoreWidget() {
  let saved = null;
  try { saved = localStorage.getItem('dashboard-widget-slot'); } catch (err) {}
...
function clearRadar() {
  stopPlayback();
  radarLayers.forEach(layer => { if (map.hasLayer(layer)) map.removeLayer(layer); });
  radarLayers = [];
  frames = [];
  slider.max = '0';
  slider.value = '0';
...
async function loadRainViewer() {
  ...
  radarDetailNote.textContent = 'RainViewer: kostenlos bis Radar-Zoom 7. Bei höherem Kartenzoom wird das Radar vergrößert, nicht detaillierter.';
  ...
  try {
    ...
    clearRadar();
    frames = next;
    radarLayers = frames.map(frame => L.tileLayer(
      `${data.host}${frame.path}/512/{z}/{x}/{y}/2/1_1.png`,
      { opacity: 0.78, zIndex: 400, maxNativeZoom: 7, maxZoom: 16, tileSize: 256, updateWhenIdle: true }
    ));
...
function loadOpenWeather() {
  ...
  radarDetailNote.textContent = 'OpenWeather: aktuelle Niederschlagskarte (keine Radar-Animation); Detailgrad hängt von Quelldaten und Tarif ab.';
  setRadarStatus('Niederschlagskarte wird geladen …');
  let seen = false;
  openWeatherLayer = L.tileLayer('https://tile.openweathermap.org/map/precipitation_new/{z}/{x}/{y}.png?appid=' + encodeURIComponent(OPENWEATHER_TEST_KEY), {
    attribution: 'Niederschlag © OpenWeather', opacity: 0.7, zIndex: 400,
    maxNativeZoom: 12, maxZoom: 16, updateWhenIdle: true
  });
  ...
  openWeatherLayer.on('tileerror', () => {
    if (radarProvider.value === 'openweather' && !seen) setRadarStatus('OpenWeather-Kacheln nicht erreichbar. API-Key und Tarif prüfen.', true);
  });
...
slider.addEventListener('input', () => { stopPlayback(); showFrame(Number(slider.value)); });
...
map.on('zoomend', () => {
  if (radarProvider.value === 'rainviewer') {
    radarDetailNote.textContent = map.getZoom() > 7
...
const weatherIcons = {
  clear: '<circle cx="9" cy="9" r="5.2"/><path d="M9 1.4V3M9 15v1.6M1.4 9H3M15 9h1.6M3.5 3.5l1.1 1.1M13.4 13.4l1.1 1.1M3.5 14.5l1.1-1.1M13.4 4.6l1.1-1.1"/>',
  cloud: '<path d="M4 15.5a3.8 3.8 0 0 1 .3-7.6 5.4 5.4 0 0 1 10.4-1.7A4.3 4.3 0 0 1 14 15.5z"/>',
  sun_cloud: '<circle cx="7" cy="7" r="3.4"/><path d="M7 1.4v1.1M7 12.5v1.1M1.4 7h1.1M12.5 7h1.1M2.9 2.9l.8.8M11.3 11.3l.8.8" opacity="0.6"/><path d="M6.5 15.5h6a3.5 3.5 0 0 0 .4-6.98A5 5 0 0 0 3.6 9.9a3 3 0 0 0 .4 5.9h2.5z" fill="currentColor" fill-opacity="0.12"/>',
  ...
  storm: '<path d="M4 12a3.8 3.8 0 0 1 .3-7.6 5.4 5.4 0 0 1 10.4-1.7A4.3 4.3 0 0 1 14 11.5z"/><path d="M9.5 13l-2.5 4h2.5l-1.5 3"/>'
};
...
const iconPressure = '<path d="M8 1v6.5M8 7.5l-2.4-2.4M8 7.5l2.4-2.4" stroke-linecap="round" stroke-linejoin="round"/><circle cx="8" cy="11" r="5.5"/>';
const iconHumidity = '<path d="M8 1.5S3 7 3 10.5a5 5 0 0 0 10 0C13 7 8 1.5 8 1.5z"/>';
const iconWind = '<path d="M1.5 6h8a2 2 0 1 0-2-2" stroke-linecap="round"/><path d="M1.5 10h10a2 2 0 1 1-2 2" stroke-linecap="round"/>';
const iconFeels = '<circle cx="8" cy="11" r="3.5"/><path d="M8 8V2a1.5 1.5 0 0 1 3 0v6a4 4 0 1 1-3 0z"/>';
const iconUv = '<circle cx="8" cy="8" r="3.2"/><path d="M8 1.3v1.6M8 13.1v1.6M1.3 8h1.6M13.1 8h1.6M3.2 3.2l1.1 1.1M11.7 11.7l1.1 1.1M3.2 12.8l1.1-1.1M11.7 4.3l1.1-1.1"/>';
const iconSun = '<path d="M2 9.5A6 6 0 0 1 14 9.5" stroke-linecap="round"/><path d="M8 9.5V3M4.5 9.5L2.5 5.5M11.5 9.5l2-4" stroke-linecap="round"/><path d="M1 12.5h14" stroke-linecap="round"/>';
...
function renderDayDetail(index) {
  ...
  const dateLabel = new Date(dateStr + 'T00:00:00').toLocaleDateString('de-DE', { weekday: 'long', day: 'numeric', month: 'long' });
  ...
  if (!rainHours.length) {
    ...
    rainSummary = rainHours.length === 1
  ...
  hoursEl.innerHTML = '';
  hourIdxForDay.filter((_, pos) => pos % 2 === 0).forEach(i => {
  ...
  if (index === 0) {
    ...
    const avgPressure = Math.round(hourIdxForDay.reduce((s, i) => s + hourly.surface_pressure[i], 0) / hourIdxForDay.length);
    const avgHumidity = Math.round(hourIdxForDay.reduce((s, i) => s + hourly.relative_humidity_2m[i], 0) / hourIdxForDay.length);
...
async function loadWeatherForPlace(lat, lon, label) {
  ...
  try {
    ...
    document.getElementById('weatherSourceNote').textContent = 'Live-Daten von Open-Meteo · Tag anklicken für Details · aktualisiert bei jeder Suche';
...
const calMonthNames = ['Januar','Februar','März','April','Mai','Juni','Juli','August','September','Oktober','November','Dezember'];
...
function renderCalendar() {
  ...
  cells.forEach(cell => {
    ...
    btn.type = 'button';
    btn.className = 'cal-day' + (cell.muted ? ' muted' : '') + (key === dateKey(today) ? ' today' : '') + (calEvents[key]?.length ? ' has-event' : '');
...
function renderCalEvents() {
  ...
  document.getElementById('calSelectedLabel').textContent = calSelectedKey === dateKey(today) ? `heute, ${label}` : label;

  const list = document.getElementById('calEvents');
  list.innerHTML = '';
  const items = (calEvents[calSelectedKey] || []).map((item, index) => ({
    index,
...
calAllDay.addEventListener('change', () => { calAddTime.disabled = calAllDay.checked; calAddTime.required = !calAllDay.checked; });
renderCalendar();
renderCalEvents();
